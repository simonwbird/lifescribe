...
import { Loader2, Heart, Clock, Pin, PinOff, Star } from 'lucide-react';
import { formatDistanceToNow } from 'date-fns';
import { useToast } from '@/hooks/use-toast';
import { cn } from '@/lib/utils';

interface GuestbookTributeBlockProps {
  personId: string;
  familyId: string;
  canModerate: boolean;
  isPublic?: boolean;
...
export function GuestbookTributeBlock({ 
  personId, 
  familyId, 
  canModerate, 
  isPublic = false,
  pageVisibility = 'private'
}: GuestbookTributeBlockProps) {
  ...
  const handleSubmit = async () => {
    ...
    if (!content.trim()) {
      toast({
        ...
        description: 'Please write your tribute message.',
        variant: 'destructive',
      });
      return;
    }
  ...
  const handlePin = async (entryId: string) => {
    await pinEntry.mutateAsync({ entryId, personId, pageType: 'tribute' });
  };
        ...
        <div className="shrink-0">
          {isFeatured ? (
            ...
            <Button
              ...
              title="Feature this tribute (max 3)"
            >
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Sign in to view and leave tributes on this memorial page.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="text-center py-8">
            <p className="text-muted-foreground mb-4">
              This is a private memorial. Please sign in to continue.
            </p>
      ...
      <CardContent className="space-y-6">
        ...
        {featuredEntries.length > 0 && (
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-semibold text-sm">Featured Tributes</h3>
            </div>
            <div className="space-y-3">
        ...
        <div className="space-y-3">
          {featuredEntries.length > 0 && regularEntries.length > 0 && (
            <h3 className="font-semibold text-sm">All Tributes</h3>
          )}
          {isLoading ? (
