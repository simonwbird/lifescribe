...
export function FamilyTreeCanvas({
  ...
  const [dragStartNodePos, setDragStartNodePos] = useState({
    x: 0,
    y: 0
  });
  const [isAltPressed, setIsAltPressed] = useState(false);
  const animationFrame = useRef<number>(0);
  const dragOverridesRef = useRef<Record<string, {
    x: number;
    y: number;
  }>>({});
  ...
  const layoutEngine = useMemo(() => {
    ...
    return new LayoutEngine(people, relationships, {
      hGap: 160,
      // horizontal spacing between cards/groups
      vGap: 220,
      // vertical spacing between generations (will be scaled by engine)
      cardWidth: 150,
      cardHeight: 180
    });
  ...
  const tick = useCallback(() => {
    ...
    if (!dragOverridesRef.current[draggingPersonId] || dragOverridesRef.current[draggingPersonId].x !== newX || dragOverridesRef.current[draggingPersonId].y !== newY) {
      dragOverridesRef.current[draggingPersonId] = {
        x: newX,
        y: newY
      };
      setVisualTick(v => v + 1);
  ...
  const handleCanvasPointerUp = () => {
    ...
    if (draggingPersonId) {
      ...
      if (override) {
        setLayoutNodes(prev => prev.map(n => n.personId === draggingPersonId ? {
          ...n,
          x: override.x,
          y: override.y
        } : n));
  ...
  const handlePersonDragStart = (e: React.PointerEvent, personId: string) => {
    ...
    if (personPos) {
      ...
      setDragOffset({
        x: 0,
        y: 0
      }); // Not used in delta mode, but keep for compatibility
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Alt') {
        ...
        setLayoutNodes(prev => prev.map(node => node.personId === selectedPersonId ? {
          ...node,
  ...
  return <div className="relative w-full h-screen bg-neutral-800 overflow-hidden">
      {/* Canvas */}
      <div ref={canvasRef} className={`w-full h-full ${isDragging && !draggingPersonId ? 'cursor-grabbing' : 'cursor-grab'}`} onPointerDown={handleCanvasPointerDown} onPointerDownCapture={handleCanvasPointerDown} onPointerMove={handleCanvasPointerMove} onPointerUp={handleCanvasPointerUp} onPointerLeave={e => {
      ...
      transformOrigin: '0 0'
        ...
        {/* Grid */}
        {showGrid && <div className="absolute inset-0 opacity-20" style={{
        backgroundImage: `
        ...
        pointerEvents: 'none' // Ensure SVG never blocks canvas panning
          ...
          console.log('🎯 FamilyTreeCanvas - Rendering ConnectionRenderer with:', {
            ...
            samplePeople: people.slice(0, 2).map(p => ({
              id: p.id,
              name: p.full_name
            })),
          ...
          <ConnectionRenderer people={people} relationships={relationships} positions={positions} />
        </svg>

        {/* Person cards - above connections (z-index: 1) */}
        <div className={`absolute inset-0 ${isDragging && !draggingPersonId ? 'pointer-events-none' : 'pointer-events-auto'} cursor-grab`} style={{
        zIndex: 1
      }}>
          {people.map(person => {
          const position = positions[person.id];
          if (!position) return null;
          return <PersonCard key={person.id} person={person} x={position.x} y={position.y} selected={selectedPersonId === person.id} isDragging={draggingPersonId === person.id || pendingDragId === person.id} onDragStart={handlePersonDragStart} onClick={handlePersonClick} onDoubleClick={handlePersonDoubleClick} />;
        })}
        </div>
      </div>
      ...
      <div className="absolute top-4 right-4 z-50 flex gap-2 pointer-events-auto">
        ...
        <Button variant="outline" size="sm" onClick={autoFit} className="bg-white/90 text-neutral-800">
        ...
        setLayoutNodes(prev => prev.map(node => {
          if (node.x < visibleBounds.minX || node.x > visibleBounds.maxX || node.y < visibleBounds.minY || node.y > visibleBounds.maxY) {
        ...
        <Dialog open={versionsDialogOpen} onOpenChange={setVersionsDialogOpen}>
          ...
          <DialogContent className="max-w-lg">
            ...
            <div className="space-y-2 max-h-96 overflow-y-auto">
              {versions.length === 0 ? <p className="text-neutral-500 text-center py-4">No saved versions yet</p> : versions.map(version => <div key={version.id} className="flex items-center justify-between p-2 border rounded">
                    <div>
                    ...
                    <div className="flex gap-1">
                      <Button variant="ghost" size="sm" onClick={() => handleLoadVersion(version)}>
                        Restore
                      </Button>
      ...
      {/* Instructions panel */}
      <div className="absolute top-4 left-4 bg-white/95 backdrop-blur-sm rounded-lg p-4 shadow-lg border max-w-xs z-50">
        ...
        <div className="space-y-2 text-xs text-neutral-600">
          ...
          <div>• Double Click persons to view their profile</div>
        </div>
      </div>
    </div>;
}