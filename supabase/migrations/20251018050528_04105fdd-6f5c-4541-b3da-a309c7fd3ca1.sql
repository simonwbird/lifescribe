-- Fix feed_for_user to load media from BOTH story_assets AND media tables
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(
      ...
      -- Fallback to media table if no story_assets
      (
        SELECT jsonb_agg(
          jsonb_build_object(
            'id', m.id,
  ...
  WHERE s.created_at < p_cursor
    ...
    AND EXISTS (
      ...
      WHERE mem.family_id = s.family_id
        AND mem.profile_id = p_user
    )
