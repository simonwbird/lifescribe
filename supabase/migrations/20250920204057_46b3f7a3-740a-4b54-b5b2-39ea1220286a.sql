-- Fix the remaining SECURITY DEFINER view by explicitly setting security_invoker=true
-- This ensures the view executes with the permissions of the querying user, not the view creator

DROP VIEW IF EXISTS public.person_timeline_items CASCADE;

-- Recreate person_timeline_items with explicit security_invoker option
