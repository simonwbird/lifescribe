-- Ensure cast to text for digest algorithm names
...
BEGIN
  ...
  calculated_hash := encode(digest(convert_to(hash_input, 'UTF8'), 'sha256'::text), 'hex');
...
BEGIN
  ...
  dedupe_key := encode(digest(convert_to(hash_input, 'UTF8'), 'sha1'::text), 'hex');
  RETURN dedupe_key;
END;
$$;
...
BEGIN
  ...
  LOOP
    hash_data := jsonb_build_object(
      'name_hash', encode(digest(convert_to(lower(trim(regexp_replace(family_record.name, '[^a-zA-Z0-9]+', '-', 'g'))), 'UTF8'), 'sha256'::text), 'hex'),
